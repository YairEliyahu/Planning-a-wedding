...
export default function TableDetailModal({ isOpen, onClose }: TableDetailModalProps) {
  ...
  const occupiedSeats = selectedTable.guests.reduce((total, guest) => total + (guest.numberOfGuests || 1), 0);
  const availableSeats = selectedTable.capacity - occupiedSeats;
  ...
  const handleRemoveGuest = (guest: any) => {
    ...
    const mainGuest = selectedTable.guests.find(g => g._id === guest._id || g._id.startsWith(guest._id.split('-companion-')[0]));
  ...
  const handleAddGuest = (guest: any) => {
    assignGuestToTable(guest, selectedTable);
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-blue-100 mt-1 font-[var(--font-heebo)]">
                    {occupiedSeats}/{selectedTable.capacity} מקומות תפוסים • {availableSeats} פנויים
                  </p>
                </div>
                <button
                  onClick={onClose}
            ...
            <div className="flex flex-col lg:flex-row h-[70vh]">
              ...
              <div className="lg:w-1/2 p-6 border-r border-gray-200">
                <h3 className="text-lg font-bold text-gray-800 mb-4 font-[var(--font-heebo)]">
                  אורחים בשולחן ({selectedTable.guests.filter(g => !g.isCompanion).length})
                </h3>
                   ...
                   >
                     <div className="text-center">
                       ...
                       <div className="text-xs text-gray-500 font-[var(--font-heebo)]">
                         {occupiedSeats}/{selectedTable.capacity}
                       </div>
                     </div>

                     {/* Guest positions around table - with names */}
                     {selectedTable.guests.map((guest, index) => {
                       const angle = (index / selectedTable.capacity) * 2 * Math.PI;
                       const radius = 60;
                     ...
                     {Array.from({ length: selectedTable.capacity - selectedTable.guests.length }).map((_, index) => {
                       const totalOccupied = selectedTable.guests.length;
                       const seatIndex = totalOccupied + index;
                       const angle = (seatIndex / selectedTable.capacity) * 2 * Math.PI;
                       const radius = 60;
                       const x = Math.cos(angle) * radius;
                       const y = Math.sin(angle) * radius;
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {selectedTable.guests.filter(g => !g.isCompanion).map((guest) => {
                    const companions = selectedTable.guests.filter(g => g.isCompanion && g._id.startsWith(guest._id));
                  ...
                  {selectedTable.guests.filter(g => !g.isCompanion).length === 0 && (
