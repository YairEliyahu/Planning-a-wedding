'use client';

import { useAuth } from '@/contexts/AuthContext';
import { useGuests } from '../context/GuestContext';

export function ConnectedAccountAlert() {
  const { user } = useAuth();
  const { forceRefresh, isLoading } = useGuests();

  if (!user?.connectedUserId) {
    return null;
  }

  const handleForceRefresh = async () => {
    try {
      await forceRefresh();
      alert('רשימת האורחים סונכרנה בהצלחה');
    } catch (error) {
      alert('שגיאה ברענון רשימת האורחים');
    }
  };

  return (
    <div className="bg-blue-50 border border-blue-200 rounded-md p-3 text-blue-800 text-sm mb-6">
      <div className="font-medium mb-1">חשבון מחובר - נתונים משותפים</div>
      <p>
        חשבון זה מחובר לחשבון של {user.partnerName || 'השותף/ה שלך'}.
        רשימת המוזמנים מסונכרנת בין שני החשבונות.
        {user.isMainEventOwner 
          ? ' אתה מוגדר כבעל החשבון הראשי.' 
          : ' השינויים שלך יופיעו גם בחשבון השותף.'}
      </p>
      <div className="flex flex-col mt-2">
        <button 
          onClick={handleForceRefresh}
          disabled={isLoading}
          className="mb-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-sm transition-colors flex items-center justify-center disabled:opacity-70"
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clipRule="evenodd" />
          </svg>
          {isLoading ? 'מסנכרן...' : 'סנכרן רשימת אורחים מלאה'}
        </button>
        <p className="text-xs text-blue-600">
          אם אתה לא רואה את כל האורחים או יש בעיות סנכרון, לחץ על כפתור הסנכרון לביצוע סנכרון מלא בין החשבונות.
        </p>
      </div>
    </div>
  );
} 